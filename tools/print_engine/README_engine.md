...
## 毎バッチの手順
1. 束を4本作る（SQL Editor で export_manifest.sql ①②。create_bundle_v2(lot_id, expires_at, 100)）
2. export_manifest.sql ③④を実行 → manifest.json に保存
3. `python card_engine.py --manifest manifest.json --out batch_0001.pdf`
   → NGなら止まる（件数・重複・旧文字 g i l o s z）。OKなら *_manifest_report.csv（c／sheet／k／lot_no／short_id／global_no／URL）が出る
4. 表紙1シートを `--sheets 1` で出してコンビニ原寸100%で確認（幅91mm）
5. 入稿（lot_no は create_bundle_v2 が rooms に入れるので後処理なし）
